...
export const AppSidebar: React.FC = () => {
  ...
  const activeDonationsCount = donations.filter((d) => d.status === 'AVAILABLE' || d.status === 'MATCHED' || d.status === 'ACCEPTED').length;
  ...
  const getBadgeValue = (key?: NavItemConfig['badgeKey']) => {
    ...
    switch (key) {
      ...
      case 'verifications':
        return 2; // mock pending verifications for admin
      case 'fraud':
        return 1; // mock critical signal for admin
      default:
        return null;
    }
  };
  ...
  const categories = ['MAIN', 'OPERATIONS', 'COMMUNICATION', 'ACCOUNT', 'SYSTEM'] as const;
  ...
  return (
    <>
      ...
      {/* Main Sidebar Card (Desktop + Mobile Drawer) */}
      <aside
        className={`fixed lg:sticky top-0 lg:top-20 left-0 h-full lg:h-[calc(100vh-6rem)] bg-white border-r lg:border border-slate-200/80 shadow-xs lg:rounded-2xl z-50 transition-all duration-300 flex flex-col justify-between select-none ${
      ...
      >
                    ...
                    >
                      ...
                      {isSidebarCollapsed && hoveredItem === item.id && (
                        <div className="absolute left-full ml-2 top-1/2 -translate-y-1/2 bg-slate-900 text-white text-xs font-medium px-3 py-1.5 rounded-lg shadow-lg z-50 whitespace-nowrap animate-in fade-in zoom-in-95 duration-100 pointer-events-none flex items-center gap-2">
                          ...
                          {badgeVal !== null && (
                            <span className="px-1.5 py-0.2 rounded-md bg-emerald-500 text-white text-[10px] font-bold">
                              {badgeVal}
                            </span>
                          )}
                        </div>
        ...
        {/* Bottom User Card / Device Trust Status */}
        {currentUser ? (
